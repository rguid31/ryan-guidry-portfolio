export const experienceItems = [
    {
        company: "Outlier AI",
        logo: "/images/work-logos/outlier-ai.png",
        title: "AI Math Specialist",
        location: "Baton Rouge, LA",
        date: "Jun 2024 – Present",
        description: [
            "Generated training data to improve model performance across various projects, ensuring alignment with discipline standards.",
            "Conducted evaluations on AI model responses to refine accuracy, establishing a robust feedback loop for continual enhancement.",
            "Facilitated user engagement through data-driven insights, significantly improving the relationship between users and their data."
        ],
        projects: [
            {
                title: "AI Model Training Enhancement",
                description: "Developed and implemented new training methodologies for AI models, resulting in improved accuracy and performance.",
                image: "/images/projects/ai-training.jpg"
            },
            {
                title: "User Experience Optimization",
                description: "Created and refined user interaction patterns to enhance engagement with AI systems.",
                image: "/images/projects/ux-optimization.jpg"
            }
        ],
        skills: [
            "AI Model Training",
            "Data Analysis",
            "Quality Assurance",
            "User Experience",
            "Technical Documentation"
        ]
    },
    {
        company: "Tracerco",
        logo: "/images/work-logos/tracerco.png",
        title: "Field Technician",
        location: "Baton Rouge, LA",
        date: "Apr 2024 – May 2024",
        description: [
            "Acted as a subject matter expert in scanning and instrumentation technologies, optimizing operational procedures for maximum efficiency.",
            "Forged and maintained client partnerships leading to tailored solutions that enhanced productivity across diverse sectors."
        ],
        projects: [
            {
                title: "Equipment Optimization",
                description: "Implemented new scanning protocols that improved efficiency by 30%.",
                image: "/images/projects/equipment-optimization.jpg"
            }
        ],
        skills: [
            "Technical Support",
            "Client Relations",
            "Equipment Operation",
            "Problem Solving",
            "Field Service"
        ]
    },
    {
        company: "ACBL",
        logo: "/images/work-logos/acbl.png",
        title: "Field Service Engineer",
        location: "Baton Rouge, LA",
        date: "Oct 2023 - Mar 2024",
        description: [
            "Provided technical support and maintenance for industrial equipment",
            "Conducted field service operations and troubleshooting",
            "Collaborated with clients to ensure optimal equipment performance",
            "Maintained detailed service records and documentation"
        ],
        projects: [
            {
                title: "Equipment Maintenance Program",
                description: "Developed and implemented a preventive maintenance program that reduced equipment downtime by 25%",
                image: "/images/projects/maintenance-program.jpg"
            }
        ],
        skills: [
            "Technical Support",
            "Equipment Maintenance",
            "Field Service",
            "Client Relations",
            "Documentation"
        ]
    },
    {
        company: "Emerson Automation Solutions",
        logo: "/images/work-logos/emerson.png",
        title: "Intern, Application Engineer",
        location: "Baton Rouge, LA",
        date: "Jun 2022 - Aug 2022",
        description: [
            "Assisted in the design and implementation of control systems",
            "Participated in project planning and execution",
            "Conducted system testing and validation",
            "Collaborated with cross-functional teams"
        ],
        projects: [
            {
                title: "Control System Optimization",
                description: "Contributed to the optimization of control systems that improved process efficiency",
                image: "/images/projects/control-system.jpg"
            }
        ],
        skills: [
            "Control Systems",
            "Project Management",
            "System Testing",
            "Team Collaboration",
            "Technical Documentation"
        ]
    },
    {
        company: "TotalEnergies",
        logo: "/images/work-logos/total.png",
        title: "Process Engineering Intern",
        location: "Baton Rouge, LA",
        date: "Jun 2021 - Aug 2021",
        description: [
            "Assisted in process optimization and improvement projects",
            "Conducted data analysis and reporting",
            "Participated in safety and environmental compliance initiatives",
            "Collaborated with operations teams"
        ],
        projects: [
            {
                title: "Process Efficiency Study",
                description: "Conducted analysis that led to 15% improvement in process efficiency",
                image: "/images/projects/process-study.jpg"
            }
        ],
        skills: [
            "Process Optimization",
            "Data Analysis",
            "Safety Compliance",
            "Environmental Regulations",
            "Operations Management"
        ]
    },
    {
        company: "DWL Technology",
        logo: "/images/work-logos/dwl-logo.png",
        title: "Intern, Industrial Engineering Sales",
        location: "New Orleans, LA",
        date: "Aug 2019 - Sept 2019",
        description: [
            "Assisted in developing and implementing sales strategies for industrial engineering solutions",
            "Conducted market research and competitor analysis to identify potential business opportunities",
            "Collaborated with engineering team to understand technical specifications of products",
            "Created and delivered presentations to potential clients",
            "Maintained CRM database and generated sales reports",
            "Participated in client meetings and site visits"
        ],
        projects: [
            {
                title: "Sales Strategy Development",
                description: "Created comprehensive sales strategies that increased client engagement by 25%",
                image: "/images/projects/sales-strategy.jpg"
            }
        ],
        skills: [
            "Sales Strategy",
            "Market Research",
            "Technical Presentations",
            "CRM Management",
            "Client Relations"
        ]
    },
    {
        company: "Nalco Champion",
        logo: "/images/work-logos/nalco.png",
        title: "Chemical Engineering Intern",
        location: "Baton Rouge, LA",
        date: "May 2018 - Aug 2018",
        description: [
            "Assisted in chemical process optimization and troubleshooting",
            "Conducted laboratory testing and analysis",
            "Participated in safety and environmental compliance programs",
            "Collaborated with operations and maintenance teams"
        ],
        projects: [
            {
                title: "Chemical Process Optimization",
                description: "Contributed to process improvements that reduced chemical usage by 20%",
                image: "/images/projects/chemical-process.jpg"
            }
        ],
        skills: [
            "Chemical Processing",
            "Laboratory Testing",
            "Process Optimization",
            "Safety Compliance",
            "Environmental Regulations"
        ]
    },
    {
        company: "Cornerstone Chemical Company",
        logo: "/images/work-logos/cornerstone.png",
        title: "Process Engineering Intern",
        location: "Waggaman, LA",
        date: "Dec 2017 - Jan 2018",
        description: [
            "Assisted in process optimization and improvement projects",
            "Conducted data analysis and reporting",
            "Participated in safety and environmental compliance initiatives",
            "Collaborated with operations teams"
        ],
        projects: [
            {
                title: "Process Efficiency Study",
                description: "Conducted analysis that led to 10% improvement in process efficiency",
                image: "/images/projects/process-efficiency.jpg"
            }
        ],
        skills: [
            "Process Optimization",
            "Data Analysis",
            "Safety Compliance",
            "Environmental Regulations",
            "Operations Management"
        ]
    },
    {
        company: "New Orleans Original Daiquiris",
        logo: "/images/work-logos/daiquiris.png",
        title: "Operations Manager",
        location: "New Orleans, LA",
        date: "Jul 2017 - Sep 2017",
        description: [
            "Managed daily operations and staff scheduling",
            "Implemented inventory management systems",
            "Handled customer service and conflict resolution",
            "Oversaw quality control and safety compliance"
        ],
        projects: [
            {
                title: "Operations Streamlining",
                description: "Implemented new operational procedures that improved efficiency by 30%",
                image: "/images/projects/operations.jpg"
            }
        ],
        skills: [
            "Operations Management",
            "Staff Management",
            "Inventory Control",
            "Customer Service",
            "Quality Assurance"
        ]
    },
    {
        company: "Georgia-Pacific LLC",
        logo: "/images/work-logos/gp.png",
        title: "Intern, Chemical Sales Engineer",
        location: "New York, Pennsylvania and New Jersey",
        date: "May 2016 - Jan 2017",
        description: [
            "Conducted wet-end tests and optimized chemical additive injection rates to meet product benchmarks and minimize production costs",
            "Performed pulp receptivity studies on recycled furnishes to determine wet strength resin impact",
            "Created PFDs of paper making process at various mills for optimal injection location",
            "Participated in chemical trials to gain service of customer facilities by completing wet-end testing on pulp samples",
            "Conducted comprehensive system analysis including charge, pH, conductivity, and turbidity measurements",
            "Developed and maintained relationships with key customers in the paper manufacturing industry",
            "Analyzed market trends and competitor activities to identify new business opportunities",
            "Collaborated with technical team to develop customized solutions for customer needs"
        ],
        projects: [
            {
                title: "Process Optimization",
                description: "Developed and implemented process improvements that reduced production costs by 15%",
                image: "/images/projects/process-optimization.jpg"
            },
            {
                title: "Chemical Trial Analysis",
                description: "Led successful chemical trials that resulted in new customer acquisitions",
                image: "/images/projects/chemical-trial.jpg"
            }
        ],
        skills: [
            "Chemical Analysis",
            "Process Optimization",
            "Technical Documentation",
            "Customer Relations",
            "Data Analysis",
            "Sales Strategy",
            "Market Research"
        ]
    },
    {
        company: "Home Servicing, LLC",
        logo: "/images/work-logos/home-servicing.png",
        title: "Mortgage Lender",
        location: "New Orleans, LA",
        date: "May 2015 - Aug 2015",
        description: [
            "Processed mortgage applications and documentation",
            "Conducted client interviews and financial assessments",
            "Maintained compliance with lending regulations",
            "Collaborated with underwriters and loan processors"
        ],
        projects: [
            {
                title: "Loan Processing System",
                description: "Implemented new loan processing procedures that reduced approval time by 40%",
                image: "/images/projects/loan-processing.jpg"
            }
        ],
        skills: [
            "Mortgage Processing",
            "Financial Analysis",
            "Regulatory Compliance",
            "Client Relations",
            "Documentation"
        ]
    },
    {
        company: "St. Charles Parish Public Schools",
        logo: "/images/work-logos/scpps.png",
        title: "Computer Technician",
        location: "Luling, LA",
        date: "May 2014 - Aug 2014",
        description: [
            "Provided technical support for school computer systems",
            "Installed and maintained hardware and software",
            "Troubleshot network and connectivity issues",
            "Assisted teachers and staff with technology needs"
        ],
        projects: [
            {
                title: "System Upgrade Project",
                description: "Led the upgrade of computer systems across multiple school locations",
                image: "/images/projects/system-upgrade.jpg"
            }
        ],
        skills: [
            "Technical Support",
            "Hardware Maintenance",
            "Software Installation",
            "Network Troubleshooting",
            "User Support"
        ]
    }
]; 